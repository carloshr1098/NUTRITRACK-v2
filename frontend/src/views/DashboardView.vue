...
export default {
  ...
  setup() {
    ...
    const getRoleTitle = () => {
      ...
      if (roles.includes('ROLE_NUTRITIONIST')) return 'Nutricionista'
      if (roles.includes('ROLE_PATIENT')) return 'Paciente'
      return 'Usuario'
    }
    ...
    const getRoleIcon = () => {
      ...
      if (roles.includes('ROLE_NUTRITIONIST')) return 'mdi-stethoscope'
      if (roles.includes('ROLE_PATIENT')) return 'mdi-account-heart'
      return 'mdi-account'
    }
    ...
    const getWelcomeMessage = () => {
      ...
      if (roles.includes('ROLE_NUTRITIONIST')) {
    ...
    const quickActions = computed(() => {
      ...
      if (roles.includes('ROLE_NUTRITIONIST')) {
        actions.push(
          { title: 'Nuevo Paciente', icon: 'mdi-account-plus', color: 'primary', route: '/nutritionist/patients' },
          { title: 'Agendar Cita', icon: 'mdi-calendar-plus', color: 'success', route: '/nutritionist/appointments' },
          { title: 'Crear Plan Dietético', icon: 'mdi-food-apple', color: 'warning', route: '/nutritionist/diet-plans' }
        )
      }

      if (roles.includes('ROLE_PATIENT')) {
        actions.push(
          { title: 'Actualizar Perfil', icon: 'mdi-account-edit', color: 'primary', route: '/patient/profile' },
          { title: 'Mis Citas', icon: 'mdi-calendar', color: 'success', route: '/patient/appointments' },
          { title: 'Mi Plan Dietético', icon: 'mdi-food', color: 'warning', route: '/patient/diet-plan' },
          { title: 'Registrar Peso', icon: 'mdi-scale', color: 'info', route: '/patient/weight-tracking' }
        )
      }
